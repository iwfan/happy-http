...
describe('As HttpParams', () => {
  ...
  it('should be able to serialize string parameters correctly', () => {
    expect(new HttpParams({ foo: 'bar', baz: 'qux' }).serialize()).toBe('foo=bar&baz=qux')
  })
  ...
  it('should be able to serialize date parameters correctly', () => {
    const date = new Date()
    expect(new HttpParams({ foo: date }).serialize()).toBe(`foo=${date.toISOString()}`)
  })
  ...
  it('should be able to serialize special character parameters correctly', () => {
    expect(
      decodeURIComponent(new HttpParams({ foo: [{ qux: 'bar' }], baz: '@:$, []' }).serialize())
  ...
  it('should have all params after merge', () => {
    ...
    expect(decodeURIComponent(p1.serialize())).toBe('foo=bar&baz=qux&baz=foo&qux={"bar":"foo"}')
  })
})
