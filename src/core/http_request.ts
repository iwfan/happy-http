...
export class HttpRequest<T = any> implements HttpRequestInit {
  ...
  merge(req: HttpRequestInit | HttpRequest): HttpRequest {
    if (isString(req.method) && this.isSupportedMethods(req.method)) {
      ;(this as Mutable<HttpRequest>).method = req.method.toUpperCase() as HttpMethods
    }
    ...
    if (isString(req.responseType) && this.isValidResponseType(req.responseType)) {
  ...
  private addContentTypeHeader(): void {
    ...
    if (isObject(this.data) || isArray(this.data) || isNumber(this.data) || isBoolean(this.data)) {
